/* Personalization Settings Tab with Memories */
...
export default function Personalization({
  ...
  if (!hasAnyPersonalizationFeature && !hasReadAccess) {
    return (
  ...
  return (
    <div className="flex h-full w-full flex-col gap-4 text-sm text-text-primary">
      {/* Memory Dropdown Section */}
      {hasReadAccess && (
        <div className="flex flex-col gap-2">
          {/* Memory Dropdown Header */}
          <button
            onClick={() => setIsMemoryExpanded(!isMemoryExpanded)}
            className="flex items-center justify-between gap-4 rounded-lg border border-border-light bg-surface-secondary p-4 text-left hover:bg-surface-hover transition-colors"
          >
            <div className="flex items-center gap-3 flex-1">
              ...
              />
              <span className="font-medium text-text-primary">{localize('com_ui_memory')}</span>
            </div>
            {hasOptOutAccess && (
              <Switch
                id="memory-toggle"
                ...
                onCheckedChange={(e) => {
                  e.stopPropagation?.();
                  handleMemoryToggle(e as unknown as boolean);
                }}
          ...
          {/* Memory Dropdown Content */}
          {isMemoryExpanded && (
            <div className="flex flex-col gap-3 rounded-lg border border-border-light bg-surface-secondary p-4">
